...
END IF;
END / / CREATE TRIGGER arc_fkarc_8_hzb_member BEFORE
...
END IF;
END / / CREATE TRIGGER arc_fkarc_8_hzb_school BEFORE
...
DELIMITER / / CREATE TRIGGER insert_member
AFTER
...
END IF;
END / / DELIMITER;
DELIMITER / / CREATE TRIGGER insert_group
AFTER
INSERT ON hzb_visitor FOR EACH ROW BEGIN
DECLARE v_visitor_id INT;
DECLARE v_size INT;
...
END IF;
END / / DELIMITER;
DELIMITER / / CREATE TRIGGER insert_individual
AFTER
INSERT ON hzb_visitor FOR EACH ROW BEGIN
DECLARE v_visitor_id INT;
...
END IF;
END / / DELIMITER;
DELIMITER / / CREATE TRIGGER apply_discount_on_parking_purchase
AFTER
INSERT ON hzb_parking FOR EACH ROW BEGIN
...
END / / DELIMITER;
DELIMITER / / CREATE TRIGGER apply_discount_on_show_purchase BEFORE
...
END IF;
END / / DELIMITER;
DELIMITER / / CREATE TRIGGER apply_discount_on_tickets_purchase
AFTER
...
END / / DELIMITER;
DELIMITER / / CREATE TRIGGER apply_discount_on_store_purchase BEFORE
INSERT ON hzb_order FOR EACH ROW BEGIN
...
END IF;
END / / DELIMITER;
DELIMITER / / CREATE PROCEDURE fetch_data(IN input_date DATE) BEGIN TRUNCATE TABLE hzb_summ;
...
VALUES (DATE '2023-05-02', 3, 30, 2, 2);
INSERT INTO hzb_order (
    ord_date,
    ord_quantity,
    ord_amount,
...
VALUES (DATE '2023-05-03', 4, 40, 3, 3);
INSERT INTO hzb_order (
    ord_date,
    ord_quantity,
    ord_amount,
...
VALUES (DATE '2023-05-04', 5, 50, 4, 4);
INSERT INTO hzb_order (
    ord_date,
    ord_quantity,
    ord_amount,
